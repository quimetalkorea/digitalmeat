...
@st.cache_data(ttl=5) # 반영 속도를 위해 캐시 시간을 5초로 줄였습니다.
def load_data():
    try:
        ...
        # [강력 정렬 로직]
        if '날짜' in df.columns:
            # 1. 날짜 열을 문자열로 강제 변환 후 점(.)이나 슬래시(/)를 대시(-)로 통일
            df['날짜_clean'] = df['날짜'].astype(str).str.replace('.', '-', regex=False).str.replace('/', '-', regex=False)
            # 2. 날짜 형식으로 변환 (errors='coerce'로 잘못된 형식은 NaT 처리)
            df['날짜_dt'] = pd.to_datetime(df['날짜_clean'], errors='coerce', fuzzy=True)
            # 3. 최신순 정렬 (내림차순 True -> 최신이 위로)
            # na_position='last'로 날짜 없는 행은 무조건 맨 아래로 보냄
...
# 3. 메인 검색창
search_input = st.text_input("🔍 검색어를 입력하세요 (예: 삼겹, 미국산, 26-01-26)", "")

if search_input and not df.empty:
    keywords = search_input.split()
    results = df.copy()
    
    # 다중 키워드 검색 (모든 단어가 포함된 행만 필터링)
    for kw in keywords:
    ...
    # 초기 화면: 정렬된 최신 데이터 20개 표시
    if not df.empty:
        st.info("검색어를 입력하시면 상세 필터가 나타납니다. (현재 최신순 정렬됨)")
        ...
        st.table(df[p_order].head(20)) # 상단 20개 노출

# 하단 정보
if not df.empty:
    st.divider()
    st.caption(f"📅 마지막 업데이트: {datetime.now().strftime('%H:%M:%S')} | 총 데이터: {len(df)}건")
